...
public class NotificationCreateDTO implements Serializable {
    ...
    @Schema(description = "是否立即推送", example = "true")
    private Boolean pushNow;

    @Schema(description = "定时推送时间", example = "2024-01-15T10:00:00")
    private LocalDateTime scheduledTime;

    @Schema(description = "目标用户ID数组（为空则推送给所有用户）", example = "[1, 2, 3]")
    private Long[] targetUserIds;

    @Schema(description = "目标角色ID数组（为空则不按角色推送）", example = "[2, 3]")
    private Long[] targetRoleIds;
